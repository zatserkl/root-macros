...
namespace Tree
{
   ...
   void book(TTree* tree, Int_t nbra=1, char type='I') {
      for (int i=0; i<nbra; ++i) {
         tree->Branch(Form("v%d",i), &v[i], Form("v%d/%c",i,type));
      }
   }
   void book(TTree* tree, std::string bnames="v", char type='I') {
      std::stringstream ss(bnames+" ");   // CINT rises error flag for last field
      std::string bname;
      for (int i=0; ss >> bname; ++i) {
         // cout<< "book branch " << bname <<endl;
         tree->Branch(Form("%s",bname.c_str()), &v[i], Form("%s/%c",bname.c_str(),type));
      }
   }
   ...
   void read(TTree* tree, const char* ifname, bool debug=false)
   {
...
TTree* tree_iarray(const char* ifname="run_158.erik", const char* ofname="")
{
   //bool debug = true;
   bool debug = false;
...
// void tree_draw(const char* ifname="tree_iarray.root")
// {
//    // uses TTree::Draw
//    
//    //bool debug = true;
...
/*
NB: inverse quotes ``!
g++ `$ROOTSYS/bin/root-config --cflags --glibs` tree_iarray.C
*/
#if !defined(__CINT__)
...
int main(int argc, char *argv[])                      // w/o int ACLiC warning ~ "no type"
{
   ...
   // tree_iarray(ifname); // NB: CINT will not see the return value

   // work in command line mode
   cout<< argv[0] << ": load macro tree_iarray.C" <<endl;
   gROOT->LoadMacro("tree_iarray.C");                        // add "+" to compile with ACLiC
   if (*ifname != '\0') gROOT->ProcessLine(Form("tree_iarray(\"%s\", \"%s\")", ifname, ofname));
   else                 gROOT->ProcessLine("tree_iarray()");
