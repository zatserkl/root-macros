// @(#)root/gpad:$Id: TCanvas.cxx 40195 2011-07-12 07:48:58Z couet $
...
Int_t canvas_max_number()
{
   // Andriy Zatserklyaniy, May 13, 2012
   ...
   {
      Int_t number = 0;
      std::sscanf(list->At(i)->GetName(), "c1_n%d", &number);
...
TCanvas::TCanvas(Bool_t build) : TPad(), fDoubleBuffer(0)
{
   ...
   if (!build || TClass::IsCallingNew() != TClass::kRealNew) {
      ...
      if (lc->FindObject(defcanvas))
      {
         //--AZ cdef = StrDup(Form("%s_n%d",defcanvas,lc->GetSize()+1));
	 Int_t number_max = canvas_max_number();
	 Int_t number = number_max+1;
	 if (number == 1) number = 2;	// where is no c1_n1
         cdef = StrDup(Form("%s_n%d",defcanvas,number));
      }
      else
         cdef = StrDup(Form("%s",defcanvas));
      Constructor(cdef, cdef, 1);
...
TObject *TCanvas::DrawClone(Option_t *option) const
{
   ...
   if (lc->FindObject(defcanvas))
   {
     //--AZ cdef = Form("%s_n%d",defcanvas,lc->GetSize()+1);
     Int_t number_max = canvas_max_number();
     Int_t number = number_max+1;
     if (number == 1) number = 2;	// where is no c1_n1
     cdef = StrDup(Form("%s_n%d",defcanvas,number));
